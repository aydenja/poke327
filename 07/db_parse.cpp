...
pokemon_stats_db p_stats [6553];
stats_db stat_names [9];
...
void db_parse(bool print)
{
  ...
  for (i = 1; i <= 6552; i++) {
    fgets(line, 800, f);
    p_stats[i].pokemon_id = atoi((tmp = next_token(line, ',')));
    tmp = next_token(NULL, ',');
    p_stats[i].stat_id = *tmp ? atoi(tmp) : -1;
    tmp = next_token(NULL, ',');
    p_stats[i].base_stat =  *tmp ? atoi(tmp) : -1;
    tmp = next_token(NULL, ',');
    p_stats[i].effort =  *tmp ? atoi(tmp) : -1;
  }
  ...
  for (i = 1; i <= 8; i++) {
    fgets(line, 800, f);
    stat_names[i].id = atoi((tmp = next_token(line, ',')));
    tmp = next_token(NULL, ',');
    stat_names[i].damage_class_id = *tmp ? atoi(tmp) : -1;
    strcpy(stat_names[i].identifier, (tmp = next_token(NULL, ',')));
    tmp = next_token(NULL, ',');
    stat_names[i].is_battle_only =  *tmp ? atoi(tmp) : -1;
    tmp = next_token(NULL, ',');
    stat_names[i].game_index =  *tmp ? atoi(tmp) : -1;
  }
  ...
  if (print) { //change me
    for (i = 0; i <= 8; i++) {
      printf("%d %d %s %d %d\n",
             stat_names[i].id,
             stat_names[i].damage_class_id,
             stat_names[i].identifier,
             stat_names[i].is_battle_only,
             stat_names[i].game_index);
    }
  }
