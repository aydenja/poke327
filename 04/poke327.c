...
int main(int argc, char *argv[])
{
  ...
  char *numS = "--numtrainers";
  int numT = 10;
  if(argc == 3){
    if(!(strcmp(argv[1], numS))){
      numT = atoi(argv[2]);
    }
  }
  printf("Adding %d trainers to map\n", numT);
  ...
  while ((p)) {
    ...
    if(is_empty(nx ,ny)){
      world.chars[cy][cx] = char_empty;
      world.chars[ny][nx] = p->ct;
      p->pos[dim_x] = nx;
      p->pos[dim_y] = ny;
    }
